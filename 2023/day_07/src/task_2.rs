...
pub fn task_2() {
    println!("Tasks 2: {}", include_str!("../input1").split("\n").map(|line| {
        let (h, s) = line.split_once(" ").unwrap();
        (Hand::new(h.chars().map(|c| match c {'T' => 10, 'J' => 1, 'Q' => 12, 'K' => 13, 'A' => 14, _ => c.to_digit(10).unwrap()}).collect()), 
        s.parse::<i32>().unwrap())
...
impl Hand {
    fn new(cards: Vec<u32>) -> Hand {
        ...
        let (m,n) = (2..15).rev().map(|i| cards.iter().filter(|&x| *x == i).count() as u8)
            .fold((0,0), |acc,j| if j > acc.0 {(j, acc.0)} else if j > acc.1 {(acc.0, j)} else {acc});
        match (5.min(m + cards.iter().filter(|&x| *x == 1).count() as u8), n) {
            (5, _) => Hand::FiveofaKind(num),
            (4, _) => Hand::FourofaKind(num),
