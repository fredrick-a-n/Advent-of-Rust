pub fn task_2() {
    println!("Tasks 2: {}", itertools::Itertools::sorted(include_str!("../input1").split("\n").map(|line| {
...
fn map_to_hand((hand, bet) :(&str,&str)) -> (Hand, i32){
    (Hand::new(hand.chars().map(|c| match c {'T' => 10, 'J' => 1, 'Q' => 12, 'K' => 13, 'A' => 14, _ => c.to_digit(10).unwrap()}).collect()), bet.parse::<i32>().unwrap())
}
...
impl Hand {
    fn new(cards: Vec<u32>) -> Hand {
        match add_jokers(get_max_counts(&cards), &cards) {
...
fn add_jokers((a,b):(u8,u8), cards: &Vec<u32>) -> (u8,u8) {
    (a + cards.iter().filter(|&x| *x == 1).count() as u8, b)
}
