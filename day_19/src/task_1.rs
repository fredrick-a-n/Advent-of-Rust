...
fn next_buy(bp: BluePrint, resources: [i32; 3], machines: [i32; 4], time: i32, to_buy: usize) -> i32 {
    ...
    if (to_buy == 3 || to_buy == 2) && time < 5 {
        return 0;
    }

    let time_needed = (0..3).map(|i| 
        (bp.costs[to_buy][i] - resources[i]).checked_div(machines[i]).unwrap_or_else(|| if bp.costs[to_buy][i] - resources[i] <= 0 {
            0
        } else {
            i32::max_value()
        })
...
pub fn task_1() {
    ...
    for i in 0..maxes.len() {
        max += maxes.get(i).unwrap() * (i+1) as i32;
    }
    println!("{}", max);
